...
// Typing Test - Single-file React component (Next.js client page compatible)
// Drop into app/typing/page.tsx or src/components/TypingTest.jsx
...
export default function TypingTestPage() {
  ...
  const containerRef = useRef<HTMLDivElement | null>(null);
    // Client-only flag for highscores rendering
    const [isClient, setIsClient] = useState(false);
    useEffect(() => { setIsClient(true); }, []);

  // SSR-safe: generate test text only on client
  ...
  const wpm = useMemo(() => {
    // standard: (correctChars/5) / minutes
  ...
  function finishTest() {
    ...
    if (startTime) {
      ...
      if (typeof window !== "undefined") {
        saveLS({ ...loadLS(), highscores: newHs });
      }
    }
  }
  ...
  function handleKey(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    // prevent default on space to keep control
    e.preventDefault();
    ...
    // if we've typed full text -> finish early
    if (pos + 1 >= testText.length) {
      finishTest();
    }
  }
  ...
  // simple spark/confetti (CSS)

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 text-gray-900 dark:text-gray-100 p-6">
      <div className="max-w-5xl mx-auto">
        <header className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">⌨️ Typing Lab — Next Gen</h1>
          <div className="text-sm opacity-80">Try to beat your best — challenge yourself!</div>
        </header>
        {/* Reserved Ad */}
        ...
        <main className="grid lg:grid-cols-[1fr,320px] gap-6">
          {/* Left: test area */}
          <section className="bg-white dark:bg-gray-900 rounded-3xl p-6 shadow-md border border-gray-200 dark:border-gray-800">
            ...
            <div className="flex gap-2 items-center mb-4">
              <select className="rounded-xl border px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" value={mode} onChange={(e) => setMode(e.target.value as any)}>
              ...
              <div className="ml-auto flex gap-2">
                <button className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => startTest()} disabled={started}>Start</button>
                <button className="px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" onClick={() => resetTest()}>Reset</button>
              </div>
            </div>
            {/* custom input area */}
            {mode === "custom" && (
            ...
            <div ref={containerRef} className="relative p-4 rounded-lg border border-gray-200 dark:border-gray-800 min-h-[160px] mb-4 bg-white dark:bg-gray-900 cursor-text">
              <HiddenInput ref={inputRef} onKey={handleKey} />
              <div className="prose break-words whitespace-pre-wrap text-lg leading-7 text-gray-900 dark:text-gray-100">
              ...
              {/* caret */}
              <div className="absolute top-3 left-3 text-xs text-gray-700 dark:text-gray-300 opacity-60">Click to focus and start typing</div>
            </div>
            {/* stats live */}
          ...
          {/* Right: analytics, heatmap, highscores */}
          <aside className="space-y-4">
            ...
            {/* SSR-safe: highscores only on client */}
            {isClient && (
              <div className="bg-white dark:bg-gray-900 rounded-3xl p-4 border border-gray-200 dark:border-gray-800">
...
function generateSpeedSeries(typed: string, elapsed: number) {
  // naive: break elapsed into buckets and compute wpm for each
